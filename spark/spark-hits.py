...
sc = SparkContext('yarn-client', "HITS") # "local", 
...
for it in xrange(10):
    ...
    adj_g1 = joined1.map(lambda x: (x[0], ( x[1][1][0] if x[1][1][0] else 0, \
                                            x[1][1][1] if x[1][1][1] else 0, \
                                            x[1][0][2],   x[1][0][3]) ))
    # key:
    #   v  = x[0]      int
    # value:
...
# RESULT
result = adj_g.takeOrdered(30, lambda x: -(x[1] + x[2]))
